...
double convert_input()
{
	...
	if (input.expsign)
		exponent = -exponent;
	if (input.point)
		exponent -= (input.mpos - input.point);
...
void apply_enter(uint16_t code)
{
	...
	case OP_ENTER_EXP:
		enter_exp();
		break;
	case OP_ENTER_BACKSPACE:
		enter_backspace();
		break;
	case OP_ENTER_UNCERT:
		enter_uncert();
		break;
	case OP_ENTER:
		enter_enter();
...
void apply_func_1to1(uint16_t code)
{
	...
	case OP_BETAGAMMA:
		...
		break;
	case OP_ETATHETA:
		f = -log(tan(trigconv * x / 2.));
		break;
	case OP_THETAETA:
		f = atan(exp(-x)) * 2 / trigconv;
		break;
	case OP_SIGNIF_X:
		if (context != CONTEXT_UNCERT)
			return;
	case OP_GAUSS_PVALUE:
		f = erfinv(1. - x) * sqrt(2.);
		break;
	default:
		break;
	...
	if (context == CONTEXT_UNCERT)
	{
		...
		{
		case OP_SIN:
			ef = trigconv * fabs(cos(trigconv * x)) * ex;
			break;
		case OP_COS:
			ef = trigconv * fabs(sin(trigconv * x)) * ex;
			break;
		case OP_TAN:
			ef = trigconv / pow(cos(trigconv * x), 2) * ex;
			break;
		case OP_ASIN:
			ef = ex / sqrt(1. - x * x) / trigconv;
			break;
		case OP_ACOS:
			ef = ex / sqrt(1. - x * x) / trigconv;
			break;
		case OP_ATAN:
			ef = ex / (1 + x * x) / trigconv;
			break;
		case OP_LG:
			ef = ex / fabs(x) / log(10.);
			break;
...
void apply_func_2to1(uint16_t code)
{
	...
	case OP_ROOTX:
		...
		break;
	case OP_CYX:
		f = exp(lgamma_r(y + 1, &gamma_sign) - lgamma_r(x + 1, &gamma_sign) - lgamma_r(y - x + 1, &gamma_sign));
		break;
	case OP_PYX:
		f = exp(lgamma_r(y + 1, &gamma_sign) - lgamma_r(y - x + 1, &gamma_sign));
		break;
	case OP_SIGNIF_XY:
		if (context != CONTEXT_UNCERT)
			return;
	case OP_POISSON:
		f = x < 0 ? NAN : exp(x * log(y) - y - lgamma_r(x + 1, &gamma_sign));
		break;
	case OP_CHI2_PROB:
		f = chisquared_cdf_c(x, y);
		break;
	default:
		break;
	}
	...
	if (context == CONTEXT_UNCERT)
	{
		...
		case OP_MINUS:
			...
			break;
		case OP_MULT:
			ef = sqrt(x * x * ey * ey + y * y * ex * ex);
			break;
		case OP_DIV:
			ef = sqrt(ey * ey / x / x + ex * ex / x / x / x / x * y * y);
			break;
		case OP_POW:
			ef = sqrt(ex * ex * pow(log(y) * pow(y, x), 2) + ey * ey * pow(x * pow(y, x - 1), 2));
			break;
		case OP_ROOTX:
			ef = sqrt(ex * ex * pow(1. / x / x * log(y) * pow(y, 1. / x), 2) + ey * ey * pow(1. / x * pow(y, 1. / x - 1), 2));
			break;
		case OP_SIGNIF_XY:
			f = (y - x) / sqrt(ex * ex + ey * ey);
			break;
		default:
			break;
		}
...
void apply_func_3to1(uint16_t code)
{
	...
	{
	case OP_PZXY:
		f = sqrt((z * z - (x + y) * (x + y)) * (z * z - (x - y) * (x - y))) / 2. / z;
		break;
	default:
		break;
	}
...
void apply_func_stat(uint16_t code)
{
	...
	case OP_STAT_CHI2:
		if (context == CONTEXT_UNCERT)
			f = stat_chi2(stack, stack2, stack_size);
		else
			return;
		break;
	}
...
void apply_const(uint16_t code)
{
	...
	case OP_CONST_PI:
		f = M_PI;
		break;
	case OP_CONST_PLANCK:
		f = 6.582119569e-22;
		break;
	case OP_CONST_PLANCKC:
		f = 197.3269804;
		break;
	case OP_CONST_E:
		f = 1.602176634e-19;
...
void apply_prog(uint16_t code)
{
	...
	case OP_STEP:
		if (program_entry)
			return;
		if (program_step == 0)
		{ // Convert input only at the start of the program
			maybe_convert_input();
			input.replace_x = 0;
		}
		...
		if (program_step >= program_size)
			program_step = 0; // Rewind to start
		break;
	case OP_REWIND:
		if (program_entry)
			return;
...
void apply_op(uint16_t code)
{
	...
	if (error_flag)
		return;
	input.replace_x = 0;
	if (opmode == OPMODE_1TO1)
		apply_func_1to1(code);
	if (opmode == OPMODE_2TO1)
		apply_func_2to1(code);
	if (opmode == OPMODE_2TO2)
		apply_func_2to2(code);
	if (opmode == OPMODE_3TO1)
		apply_func_3to1(code);
	if (opmode == OPMODE_STAT)
		apply_func_stat(code);
...
void enter_key(uint16_t code, uint16_t code1, uint16_t code2)
{
	...
	else if (shift == 2)
	{
		apply_op(code2);
		shift = 0;
		if ((code2 & OPMODE_MASK) != OPMODE_TEST)
			draw_flags |= DRAW_STATUS;
	}
}
...
int calc_on_key(int c)
{
	...
	case 7:
		...
		break;
	case 8:
		enter_key(OP_ENTER_2, OP_NOP, OP_CONST_PLANCK);
		break;
	case 9:
		enter_key(OP_ENTER_3, OP_NOP, OP_CONST_PLANCKC);
		break;
	case 13:
		enter_key(OP_ENTER_4, OP_POISSON, OP_CONST_E);
		break;
	case 14:
		enter_key(OP_ENTER_5, OP_CHI2_PROB, OP_CONST_NA);
		break;
	case 15:
		enter_key(OP_ENTER_6, OP_GAUSS_PVALUE, OP_CONST_K);
		break;
	case 19:
		enter_key(OP_ENTER_7, OP_ETATHETA, OP_THETAETA);
		break;
	case 20:
		enter_key(OP_ENTER_8, OP_GAMMABETA, OP_BETAGAMMA);
		break;
	case 21:
		enter_key(OP_ENTER_9, OP_PZXY, OP_NOP);
		break;
	...
	case 3:
		...
		break;
	case 4:
		enter_key(OP_ENTER_UNCERT, OP_NOP, OP_NOP);
		break;
	case 5:
		enter_key(OP_NOP, OP_NOP, OP_NOP);
		break;
	case 22:
		enter_key(OP_ENTER_EXP, OP_STAT_CHI2, OP_NOP);
		break;
	case 23:
		enter_key(OP_ENTER_BACKSPACE, OP_CLEAR_STACK, OP_CLEAR_MEM);
		break;

	case 16:
		enter_key(OP_MULT, OP_STAT_MEAN, OP_NOP);
		break;
	case 17:
		enter_key(OP_DIV, OP_NOP, OP_NOP);
		break;
	case 10:
		enter_key(OP_PLUS, OP_SIGNIF_X, OP_NOP);
		break;
	case 11:
		enter_key(OP_MINUS, OP_SIGNIF_XY, OP_NOP);
		break;

	case 25:
		enter_key(OP_PROG, OP_REWIND, OP_NOP);
		break;
	case 26:
		enter_key(OP_RUN, OP_STEP, OP_NOP);
		break;
	case 27:
		enter_key(OP_NOP, OP_NOP, OP_NOP);
		break;
	case 28:
		...
		break;
	case 29:
		enter_key(OP_ENTER, OP_NOP, OP_NOP);
		break;

	case 31:
		enter_key(OP_SQRT, OP_SQR, OP_TEST_7);
		break;
	case 32:
		enter_key(OP_INV, OP_FACTORIAL, OP_NOP);
		break;
	case 33:
		enter_key(OP_ERF, OP_ERFINV, OP_NOP);
		break;
	case 34:
		enter_key(OP_POLAR, OP_DESCARTES, OP_NOP);
		break;
	case 35:
		enter_key(OP_GAMMA, OP_LOGGAMMA, OP_NOP);
		break;
	case 36:
		enter_key(OP_CYX, OP_PYX, OP_NOP);
		break;

	case 37:
		enter_key(OP_POW, OP_ROOTX, OP_TEST_1);
		break;
	case 38:
		enter_key(OP_LN, OP_EXP, OP_TEST_2);
		break;
	case 39:
		enter_key(OP_LG, OP_POW10, OP_TEST_3);
		break;
	case 40:
		enter_key(OP_SIN, OP_ASIN, OP_TEST_4);
		break;
	case 41:
		enter_key(OP_COS, OP_ACOS, OP_TEST_5);
		break;
	case 42:
		enter_key(OP_TAN, OP_ATAN, OP_TEST_6);
		break;

	case 43:
		enter_key(OP_DROP, OP_ROTUP, OP_ROTDOWN);
		break;
	case 44:
		enter_key(OP_SWAP, OP_LASTX, OP_NOP);
		break;
	case 45:
		change_trigmode();
		break;
	case 46:
		enter_key(OPMODE_MPLUS, OPMODE_MMINUS, OP_NOP);
		break;
	case 47:
		enter_key(OPMODE_RCL, OPMODE_RCLX, OP_NOP);
		break;
	case 48:
		enter_key(OPMODE_STO, OPMODE_STOX, OP_NOP);
		break;
	...
	// clear_cursor();
	if (!(input.started && input.replace_x == 0))
		clear_cursor();
	if (draw_flags & DRAW_STACK)
		draw_stack();
	...
	if (draw_flags & DRAW_CURSOR)
		draw_cursor();
	if (draw_flags & DRAW_ERROR)
		draw_error(0, 1);
	if ((draw_flags & DRAW_STATUS) || (draw_flags & DRAW_STACK))
		draw_status();
